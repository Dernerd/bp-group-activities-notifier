...
function bp_local_group_notifier_format_notifications( $action, $item_id, $secondary_item_id, $total_items, $format = 'string' ) {
	...
	if ( (int) $total_items > 1 ) {
		...
		$text = sprintf( __( '%1$d neue Aktivitäten in der Gruppe "%2$s"', 'bp-group-activities-notifier' ), (int) $total_items, $group->name );

		if ( 'string' == $format ) {
			return '<a href="' . $group_link . '" title="' . __( 'Neue Gruppenaktivitäten', 'bp-group-activities-notifier' ) . '">' . $text . '</a>';
		} else {
			return array(
				'link' => $group_link,
				'text' => $text,
